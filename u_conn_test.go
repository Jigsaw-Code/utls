...
func runUTLSClientTestForVersion(t *testing.T, template *clientTest, prefix, option string, parrotId ParrotID) {
	test := *template
	...
	test.runUTLS(t, *update, parrotId)
}

func runUTLSClientTestTLS12(t *testing.T, template *clientTest, parrotId ParrotID) {
	runUTLSClientTestForVersion(t, template, "TLSv12-", "-tls1_2", parrotId)
}

func (test *clientTest) runUTLS(t *testing.T, write bool, parrotId ParrotID) {
	...
	config := test.config
	if config == nil {
		t.Errorf("Explicit config is mandatory")
		return
	}
	...
	go func() {
		...
		extConf, err := client.MakeExtendedConfigParrotID(parrotId)
		if err != nil {
			t.Errorf("Failed to parrot by ID %+v: %s", parrotId, err)
			return
		}
		state, err := extConf.BuildState()
		if err != nil {
			t.Errorf("Failed to build state from extended config: %s", err)
			return
		}
		err = client.ClientHandshakeWithState(state)
		if err != nil {
			t.Errorf("ClientHandshakeWithState failed: %s", err)
			return
		}
...
func TestUTLSHandshakeClientParrotAndroid_5_1(t *testing.T) {
	parrotId := ParrotAndroid_5_1_Browser
	...
	// testUTLSHandshakeClientECDHE_ECDSA_WITH_CHACHA20_POLY1305(t, parrotId)
	// testUTLSHandshakeClientECDHE_RSA_WITH_CHACHA20_POLY1305(t, parrotId)

	testUTLSHandshakeClientECDHE_RSA_AES128_GCM_SHA256(t, parrotId)
	testUTLSHandshakeClientECDHE_ECDSA_AES128_GCM_SHA256(t, parrotId)
	testUTLSHandshakeClientECDHE_RSA_AES256_CBC_SHA(t, parrotId)
	testUTLSHandshakeClientECDHE_ECDSA_AES256_CBC_SHA(t, parrotId)
	testUTLSHandshakeClientECDHE_RSA_AES128_CBC_SHA(t, parrotId)
	testUTLSHandshakeClientECDHE_ECDSA_AES128_CBC_SHA(t, parrotId)

	testUTLSHandshakeClientRSA_AES128_GCM_SHA256(t, parrotId)
}

// Enable whenever EMS is implemented
func disabledtestUTLSHandshakeClientParrotAndroid_6_0(t *testing.T) {
	parrotId := ParrotAndroid_6_0_Browser
...
func disabledtestUTLSHandshakeClientParrotChrome_58(t *testing.T) {
	...
	testUTLSHandshakeClientECDHE_ECDSA_AES128_GCM_SHA256(t, parrotId)
	testUTLSHandshakeClientECDHE_RSA_AES128_GCM_SHA256(t, parrotId)
	testUTLSHandshakeClientECDHE_ECDSA_AES256_GCM_SHA256(t, parrotId)
	testUTLSHandshakeClientECDHE_RSA_AES256_GCM_SHA256(t, parrotId)
...
func testUTLSHandshakeClientECDHE_RSA_AES128_CBC_SHA(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
	opensslCipherName := "ECDHE-RSA-AES128-SHA"
	test := &clientTest{
		name:    "UTLS-" + opensslCipherName + "-" + parrotId.Str(),
		command: []string{"openssl", "s_server", "-cipher", opensslCipherName},
		config:  config,
	}

	runUTLSClientTestTLS12(t, test, parrotId)
}

func testUTLSHandshakeClientECDHE_RSA_AES256_CBC_SHA(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_ECDSA_AES128_CBC_SHA(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_ECDSA_AES256_CBC_SHA(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientRSA_AES128_GCM_SHA256(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_ECDSA_AES128_GCM_SHA256(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_RSA_AES128_GCM_SHA256(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_ECDSA_AES256_GCM_SHA256(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_RSA_AES256_GCM_SHA256(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_RSA_WITH_CHACHA20_POLY1305(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
...
func testUTLSHandshakeClientECDHE_ECDSA_WITH_CHACHA20_POLY1305(t *testing.T, parrotId ParrotID) {
	config := getUTLSTestConfig()
